# Home Page — Implementation Plan (EXAMPLE ONLY)

> **This file is a reference walkthrough**, not boilerplate defaults.  
> For a new project: copy [plan-template.md](../plan-template.md) → `docs/{PAGE}_PAGE_PLAN.md` and use your Figma fileKey.  
> Kept here as a complete example of the Figma → Payload agent workflow.
...
```
Pages (home)
├── hero: { type: 'marketingHero', headline, media, ... }
└── layout: [
      { blockType: 'logoCloud', ... },
      { blockType: 'benefits', ... },
...
### 4.2 Hero — `marketingHero` variant (new)
...
**Seed headline:** `"Browse everything."` (demo — keep Figma copy or customize)
...
**Figma node:** `1:188` — "Why Choose Area?" → **"Why Choose Site?"**
...
flowchart TD
  ...
  P0 --> P1B[Phase 1B: Marketing Hero ✅]
...
### Phase 1B — Marketing Hero (1 subagent)
...
| SiteButton padding | 22×14px, 6px arrow | 24×12px, 16px arrow | Nav, CTAs |
...
|------|-----------|-------|---------|
| 1 | MarketingHero | Headline→green bar gap: Figma 240/140/120px, code 24–40px | `src/heros/MarketingHero/index.tsx` |
...
- SiteButton: `px-[22px] py-[14px]`, arrow 6×6px
...
| `phase-6b-hero` | MarketingHero | H1 typography; headline→bar gaps; padding; mobile nav clearance | `src/heros/MarketingHero/` |
...
Plan: docs/EXAMPLE_PAGE_PLAN.md — Phase 6{X}, §5B backlog
...
```
docs/
  EXAMPLE_PAGE_PLAN.md          ← this file
...
1. Figma MCP get_screenshot(nodeId) @ 1280 / 800 / 375
   → references/figma/demo-home/{breakpoint}/{section}.png
...
| Item | Decision | Notes |
|------|----------|-------|
| Brand copy | Replace "Area" → "Site" in seed; keep competitor names | POC |
...
- [ ] colors and fonts match Figma tokens (± reasonable POC tolerance) — **partial; Phase 6 targets full token alignment**
...
- [x] MarketingHero headline→bar gap matches Figma (240/140/120px)
