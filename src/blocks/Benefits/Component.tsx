...
export const BenefitsBlock: React.FC<BenefitsBlockProps> = ({
  ...
  return (
    <section
      className="bg-site-bg pb-[120px]"
      data-testid="block-benefits"
      id={SECTION_ANCHORS.benefits}
    >
      <SiteSection as="div" className="flex flex-col">
        <div className="flex flex-col gap-[50px] border-t border-site-divider pb-[60px] pt-20">
          ...
          {items && items.length > 0 && (
            <div className="grid grid-cols-1 gap-5 pt-10 md:grid-cols-2 lg:grid-cols-4">
              {items.map((item, index) => (
                <article
                  className="flex flex-col gap-6 border-t border-site-divider py-10"
                  key={item.id || index}
                >
                  <Icon className="size-6 text-site-text" name={item.icon} />
